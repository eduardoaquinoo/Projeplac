...
import { getPublishedProjects } from "../utils/projectsManager";

interface CategoriesProps {
  onNavigateToProjects?: (course?: string) => void;
}
...
export function Categories({ onNavigateToProjects }: CategoriesProps) {
  const [categories, setCategories] = useState(categoriesBase.map(cat => ({ ...cat, count: 0 })));

  useEffect(() => {
    // Carregar projetos e contar por curso
    const publishedProjects = getPublishedProjects();
    const categoriesWithCount = categoriesBase.map(cat => {
      const count = publishedProjects.filter(p => p.course === cat.name).length;
      return { ...cat, count };
    });
